...
public class FileOperation {
    ...
    boolean newFileFlag;
    String fileName;
    String applicationTitle = "Notepad - Summit Academy OIC";
    ...
    /////////////////////////
// Constructor
// Creates a new file chooser to allow
// the user to open and save files
...
// Displays a dialog to allow the user to choose the file.
    void openFile()
    {
        if(!confirmSave()) {
            return;
        ...
        do
        {
            ...
            JOptionPane.showMessageDialog(this.notepad.frame,
                    "<html>"+file1.getName()+"<br>file not found.<br>"+
        ...
        //If the file can't be written to yet, set the new file flag to tru
        if(!file1.canWrite()) {
            newFileFlag = true;
        }//end if
...
// Updates the file status while the file is saved
    ...
    ///////////////////////
// Shows an option pane dialog to prompt the user to confirm that they want to sa ve the file.
    boolean confirmSave()
    {
        ...
        if(!saved)
        {
            int x=JOptionPane.showConfirmDialog(this.notepad.frame,strMsg,applicationTitle,
